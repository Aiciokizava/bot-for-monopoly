							...
							if ($player_info[5]) {
								...
								switch ($field_info[0]) {
									...
									case 'jackpot':
										...
										$monopoly->assembling_a_game_card(
											$peer_id,
											$message . "Вы попадаете в казино.\nВыберите от 1 до 3 чисел и испытайте удачу. Если угадаете выпавшее число, то получите выигрыш.\n\nНа кубиках выпало: $dice[0] и $dice[1]\nСуммарно: " . array_sum($dice) . "\n" . $monopoly->player_balance($peer_id),
											'{"inline":true,"buttons":[[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 1"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 2"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 3"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 4"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 5"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 6"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Взаимодействие"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Отказаться"},"color":"negative"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Поставить 1,000k"},"color":"positive"}]]}'
										);
										break;
									case 'police':
										$mysqli->query("UPDATE `monopoly`.`{$peer_id}_players` SET `cell` = '11', `arrest` = '1' WHERE `user_id` = $user_id;");
						...
						if (!is_array($checking_game_conditions)) {
							...
							if (count($player_info[6]->element) < 4) {
								...
								if (count($player_info[6]->element)) {
									$vk->messages_send(
										$peer_id,
										'Вы выбрали следующие числа: ' . implode(', ', $player_info[6]->element),
										'',
										'{"inline":true,"buttons":[[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 1"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 2"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 3"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 4"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 5"},"color":"secondary"},{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"&#127922; 6"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Взаимодействие"},"color":"secondary"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Отказаться"},"color":"negative"}],[{"action":{"type":"text","payload":"{\"button\":\"1\"}","label":"Поставить 1,000k"},"color":"positive"}]]}'
									);
								} else {
